...
const Message = ({ messages }) => {
  ...
  return (
    <>
      <div
        ref={ref}
        className={`message   object-scale-downn    overflow-y-auto p-3 flex items-center p-2.5 ${messages.senderId === currentUser.uid ? "justify-end " : "justify-start"} ${messages.senderId === currentUser.uid && "owner"}  `}
      >
        <div
          className={`  w-12 h-12 rounded-full bg-rose-400 p-0.5  flex justify-start ${messages.senderId === currentUser.uid ? "hidden" : ""} `}
        >
          <img
